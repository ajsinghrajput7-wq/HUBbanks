...
const HubBankChart: React.FC<HubBankChartProps> = ({
  ...
  // Connected flight IDs for highlighting
  ...
  const handleFlightClick = (e: React.MouseEvent, f: FlightInfo) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      // Multi-select
      setSelectedFlights(prev => prev.some(sf => sf.id === f.id)
        ? prev.filter(sf => sf.id !== f.id)
        : [...prev, f]
      );
      setPinnedFlight(null);
    } else {
      // Single pin — open intel card
  ...
  const generateAI = async () => {
    ...
    try {
      const { outbound, inbound } = getTwoWaySummary(focal);
      const topOut = outbound.slice(0, 12).map(f => `${f.code}(${f.freq})`).join(', ');
      const topIn = inbound.slice(0, 12).map(f => `${f.code}(${f.freq})`).join(', ');
      const connPorts = new Set([...outbound, ...inbound].map(f => f.code));
      const prompt = `You are a senior aviation network strategist analyzing hub connectivity at BLR (Bengaluru).\n\nFlight: ${focal.code} (${focal.airline || 'unknown airline'}) — Hub time: ${focal.exactTime || focal.originalHubTime}\nMCT: ${mct}h | Connection window: ${maxConnectionWindow}h\nOutbound connections: ${topOut || 'none'}\nInbound feeders: ${topIn || 'none'}\nTotal connectable ports: ${connPorts.size}\n\nProvide a concise 3-point strategic assessment:\n1. Connection quality and hub value\n2. Key gaps or underserved markets\n3. Specific optimization recommendation\n\nBe specific, use airport codes, be actionable.`;
  ...
  return (
    <div className="h-full flex flex-col gap-2 relative" onClick={() => { setPinnedFlight(null); setSelectedFlights([]); }}>
        ...
        return (
          <div className="fixed top-20 right-4 w-80 bg-[#1a2332] rounded-2xl shadow-2xl overflow-hidden z-50 border border-slate-700/50"
            ...
            <div className="grid grid-cols-3 gap-px mx-4 mb-3 bg-slate-600 rounded-xl overflow-hidden">
              {[
                { label: 'OPS', val: totalOps > 0 ? totalOps.toLocaleString('en-IN') : (freqMode === 'weekly' ? `${pinnedFlight.freq}` : (pinnedFlight.freq / 7).toFixed(1)) },
                    ...
                    {items.length === 0
                      ...
                      : items.map((it, i) => (
                        <div key={i} className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-[9px] font-bold text-slate-400 tabular-nums">{it.freq}</span>
                        </div>
                      ))}
                  </div>
                ))}
      ...
      {analyzerOpen && (pinnedFlight || selectedFlights.length >= 1) && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-[#0d1520] z-[100] flex flex-col overflow-hidden">
            {/* Analyzer header */}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 bg-slate-800 rounded-lg px-2 py-1">
                  ...
                  {['ALL', 'DOMESTIC', 'INTERNATIONAL'].map(m => (
                    <button key={m} className={`px-2 py-0.5 rounded text-[8px] font-black uppercase transition-all ${m === 'ALL' ? 'bg-white text-slate-900' : 'text-slate-500 hover:text-white'}`}>{m}</button>
                  ))}
                </div>
                {/* Sub MCT toggle */}
                <div className="flex items-center gap-1.5 bg-slate-800 rounded-lg px-3 py-1.5">
                  ...
                  <button onClick={() => setSubMct(!subMct)} className={`relative inline-flex h-3.5 w-7 items-center rounded-full transition-colors ${subMct ? 'bg-indigo-500' : 'bg-slate-600'}`}>
                ...
                <div className="flex items-center bg-slate-800 rounded-xl p-0.5 gap-0.5">
                  {[
                    ...
                    <button key={tab.key}
                      onClick={() => { setAnalyzerTab(tab.key as any); if (tab.key === 'ai' && !aiInsight) generateAI(); }}
                          ...
                          <div key={i} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                            <div className="px-3 pt-3 pb-2 flex items-start justify-between">
                              <div>
                                ...
                                <p className="text-[8px] text-slate-400 font-bold uppercase">{region} · <span className="text-emerald-600">↑{port.freq}</span></p>
                              </div>
                              <span className="text-[7px] bg-slate-100 text-slate-500 font-black rounded px-1 py-0.5 uppercase">
                                {portFlights.reduce((s, f) => s + f.freq, 0)}/wk
                              </span>
                            </div>
                                ...
                                {outFlights.map((f, fi) => {
                                  ...
                                  return (
                                    <div key={fi} className="flex items-center justify-between px-1 py-0.5">
                                      ...
                                      <span className="text-[9px] font-black text-slate-700">{f.freq}</span>
                                    </div>
                                  );
                                })}
                              </div>
