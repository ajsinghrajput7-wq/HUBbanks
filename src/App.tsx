...
const App: React.FC = () => {
  ...
  // Pre-loaded dataset registry — add new entries here as you acquire more data
  const PRELOADED_DATASETS = [
    { id: 'blr-jun26', code: 'BLR', period: 'June 2026', label: 'BLR — June 2026', fileName: 'BLRbank_june26_JobId3667673.csv', url: '/data/BLRbank_june26_JobId3667673.csv' },
  ];

  // Parse the dual-sided OAG/IATA bank CSV format:
  ...
  const parseCSV = (text: string): any[] => {
    ...
    rawRows.slice(1).forEach(raw => {
      ...
      if (c[0]?.trim()) {
        // ARRIVAL row — left side populated
        const origin = c[2]?.trim();
        if (!origin || origin.length < 3) return;
        results.push({
          arrivalAirline:   c[0].trim(),
          arrivalFlightNo:  c[1].trim(),
          arrivalCode:      origin,
          arrivalFreq:      parseFreq(c[3]),
          arrivalSeats:     safeInt(c[6]),
          hub_time:         hubTime,
          departureCode:    '',
          departureAirline: '',
          departureFlightNo:'',
        ...
        // DEPARTURE row — right side populated
        ...
        results.push({
          ...
          hub_time:         hubTime,
          departureCode:    dest,
          departureAirline: c[16].trim(),
          departureFlightNo:c[15].trim(),
          departureFreq:    parseFreq(c[13]),
          departureSeats:   safeInt(c[10]),
          arrivalTime:      c[9]?.trim() || '',
        });
      }
    });
  ...
  const loadPreloaded = async (entry: typeof PRELOADED_DATASETS[0]) => {
    ...
    try {
      ...
      if (parsed.length > 0) {
        const ds: AirportDataset & { period?: string } = {
          id: entry.id,
          code: entry.code,
      ...
      alert('Failed to load pre-loaded dataset. Try uploading the CSV manually.');
    }
    setLoading(false);
  };
