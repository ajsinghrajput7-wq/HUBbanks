...
const App: React.FC = () => {
  ...
  const parseCSV = (text: string): any[] => {
    ...
    const fi = (kws: string[]) => { let i = headers.findIndex(h => kws.some(k => h === k)); if (i !== -1) return i; return headers.findIndex(h => kws.some(k => h.includes(k))); };
    const seatIdxs = headers.reduce((a:number[],h,i) => h.includes('seats')?[...a,i]:a, []);
    const paxIdxs = headers.reduce((a:number[],h,i) => h.includes('pax')?[...a,i]:a, []);
    const hm = { airline: fi(['airline']), origin: fi(['origin','from']), depTime: fi(['departure time']), hubTime: fi(['hub time']), arrTime: fi(['arrival time']), arrival: fi(['arrival','destination','to']) };
    return rows.slice(1).map(row => {
      const c = row.split(',').map(x => x.trim());
      return { arrivalAirline:c[hm.airline]||'', arrivalFlightNo:c[1]||'', arrivalCode:c[hm.origin]||'', arrivalFreq:(c[3]?.match(/[1-7]/g)||[]).length, arrivalSeats:parseInt(c[seatIdxs[0]])||0, arrivalPax:parseInt(c[paxIdxs[0]])||0, arrivalTime:c[hm.depTime]||'', hub_time:c[hm.hubTime]||'', departureCode:c[hm.arrival]||'', departureTime:c[hm.arrTime]||'', departureFreq:(c[13]?.match(/[1-7]/g)||[]).length, departureSeats:parseInt(c[seatIdxs[1]])||0, departurePax:parseInt(c[paxIdxs[1]])||0, departureFlightNo:c[c.length-2]||'', departureAirline:c[16]||c[c.length-1]||'' };
    }).filter(r => !!r.hub_time);
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    const nd: AirportDataset[] = [];
    for (const f of files) {
      const text = await f.text();
      const parsed = parseCSV(text);
      if (parsed.length > 0) {
        const match = f.name.toUpperCase().match(/[A-Z]{3}/);
        nd.push({ id: Math.random().toString(36).slice(2), code: match?.[0]||'UNK', fileName: f.name, data: parsed });
      }
    }
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50 font-sans">
        ...
        <div className="bg-slate-50 px-5 border-b border-slate-200 flex items-center gap-0.5 overflow-x-auto no-scrollbar shrink-0 h-9">
          {datasets.map(d => (
            <button key={d.id} onClick={() => setActiveAirportId(d.id)}
              ...
              {d.code}
              <span onClick={e => removeDataset(d.id, e)} className="hover:text-red-500 cursor-pointer ml-0.5">✕</span>
            </button>
          ))}
        </div>
      )}
      ...
      <main className="flex-1 overflow-hidden bg-slate-100">
        {loading ? (
          ...
          <div className="h-full flex flex-col items-center justify-center m-6 border-2 border-dashed border-slate-300 rounded-3xl bg-white/60">
            <div className="text-center max-w-sm">
              <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <svg width="24" height="24" fill="none" stroke="#94a3b8" strokeWidth="2" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
              </div>
              <h2 className="text-sm font-black text-slate-700 uppercase mb-2">Upload your schedule CSV</h2>
              <p className="text-xs text-slate-400 leading-relaxed mb-4">Supports OAG/IATA schedule exports. Name your file with the airport code (e.g. BLR.csv) for automatic detection.</p>
              <label className="inline-flex items-center gap-2 bg-[#006a4e] text-white px-4 py-2 rounded-xl cursor-pointer text-xs font-black uppercase hover:bg-[#005a40]">
