...
const App: React.FC = () => {
  ...
  // Map pre-loaded IDs to their bundled raw CSV strings
  const BUNDLED_DATA: Record<string, string> = {
    'blr-jun26': blrJun26Raw,
  };

  const loadPreloaded = (entry: typeof PRELOADED_DATASETS[0]) => {
    ...
    setLoading(true);
    try {
      const raw = BUNDLED_DATA[entry.id];
      if (!raw) throw new Error('Bundled data not found');
